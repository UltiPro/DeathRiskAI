...
def feature_transformation(
    df: pd.DataFrame, test_size: float = 0.1, random_state: int = RANDOM_SEED
) -> None:
    ...
    Y_trainval.to_frame(name="death").to_parquet("./trainval_test_data/Y_trainval.parquet", index=False)

    # Save the testing set
    print("💾 Saving testing set...")
    X_test.to_parquet("./trainval_test_data/X_test.parquet", index=False)
    Y_test.to_frame(name="death").to_parquet("./trainval_test_data/Y_test.parquet", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_size", type=float, default=0.1, help="Testing set size (default: 0.1)")
    parser.add_argument(
        "--random_state",
        type=int,
        default=42,
